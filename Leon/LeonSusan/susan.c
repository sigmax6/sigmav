...
/*#define HGEN             [> Generate image.h file <]*/
#define LEON           /* Susan Leon port flag */
...
#ifdef LEON
	printf("SUSAN Leon test port by SSDUT sigmax6 2011\n ");
#else
	exit(0);
#endif
}
...
#ifdef HGEN
	FILE * fd_out;
	if((fd_out=fopen("image.h","w"))==NULL)
	{
		printf("Can't open image.h\n");
	}

	/*fwrite(*in,1,*x_size * *y_size,fd_out);*/
	/*uchar * in_ptr = *in;*/
	int counter = 0;
	int image_size = *x_size * *y_size;
	fprintf(fd_out,"static uchar image_date [%d] = {",image_size);
...
#else
		usage();
		printf("Processing flags :");
		for (argindex=0;argindex<argc_in;argindex++)
		{
			printf("%s ",argv_in[argindex]);
		}
		printf("For LEON \n\n");
		argindex = 0;
		while (argindex < argc_in)
		{
			tcp = argv_in[argindex];
#endif
			...
			/* }}} */
			/* {{{ main processing */
			printf("Starting susan ,please wait . \n Timer started!\n\n ");
			clock_t time = clock();
